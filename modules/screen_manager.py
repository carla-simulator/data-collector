...
class ScreenManager(object):
    ...
    def draw_wheel_on(self, steer):
        ...
        # scale = 0.5
        position = (3 * self._resolution[0] / 2 - cols / 2, int(self._resolution[1] / 1.5) - rows / 2)
        # print position
    ...
    def plot_camera_steer(self, sensor_data, control, screen_position=[0, 0], status=None):
        ...
        if status is not None:
            draw_vbar_on(sensor_data, acc, int(1.5 * size_x / 8), (0, 255, 0))

            draw_vbar_on(sensor_data, brake, int(1.5 * size_x / 8) + 97, (255, 0, 0))
        ...
        if status is not None:
            ...
            direction_pos = (int(size_y / 2 - size_x / 2 - size_x / 13), int(size_x / 2 - size_x / 4))
            self.paint_on_screen(int(size_x / 8), str(status['directions']), (0, 255, 0), direction_pos, screen_position)
            direction_pos = (size_y / 2 - size_x / 2 - size_x / 13, size_x / 2 - size_x / 4 + size_x/6)
            self.paint_on_screen(int(size_x / 8), str(status['stop_pedestrian']),
                                 (0, 255, 0), direction_pos, screen_position)
            direction_pos = (size_y / 2 - size_x / 2 - size_x / 13, size_x / 2 - size_x / 4 + 2*size_x/6)
            self.paint_on_screen(int(size_x / 8), str(status['stop_vehicle']),
                                 (0, 255, 0), direction_pos, screen_position)
            direction_pos = (size_y / 2 - size_x / 2 - size_x / 13, size_x / 2 - size_x / 4 + 3*size_x/6)
            self.paint_on_screen(int(size_x / 8), str(status['stop_traffic_lights']),
                                 (0, 255, 0), direction_pos, screen_position)
